...
async def test_embed_posts_single_input_and_returns_vector():
    ...
    embedder = EmbeddingServiceClient(BASE_URL, http_client=mock_client(handler))
...
async def test_satisfies_both_embedder_ports():
    embedder = EmbeddingServiceClient(BASE_URL, http_client=mock_client(lambda _r: httpx.Response(200, json=[[0.0]])))
...
async def test_server_error_propagates():
    embedder = EmbeddingServiceClient(BASE_URL, http_client=mock_client(lambda _r: httpx.Response(503)))
